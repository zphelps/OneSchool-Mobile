...
class OpponentSearchNotifier extends ChangeNotifier{

  List<OpponentModel> _data = [];
  List<OpponentModel> get data => _data;
  ...
  Future<void> getData(mounted, String? query, bool Function(OpponentModel)? filter) async {
    QuerySnapshot rawData;
    currentQuery = query;

    if (lastVisible == null) {
      rawData = await FirebaseFirestore.instance
          .collection('tenants').doc(FBDatabase.tenantID).collection('opponents')
    ...
    if (rawData.docs.isNotEmpty) {
      ...
      if (mounted) {
        ...
        _data = _snap.where((element) {
          ...
          return true;
        }).map((e) => OpponentModel.fromMap(e.data() as Map<String, dynamic>)).toList();
        if(filter != null) {
  ...
  onRefresh(mounted, String? query, bool Function(OpponentModel)? filter) {
    _isLoading = true;
    _snap.clear();
    _data.clear();
    _lastVisible = null;
