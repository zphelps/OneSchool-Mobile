...
class ManageOwners extends ConsumerStatefulWidget {
  final AppConfiguration prefs;
  final String groupID;
  const ManageOwners({Key? key, required this.prefs, required this.groupID}) : super(key: key);

  @override
  ConsumerState<ManageOwners> createState() => _ManageOwnersState();
}

class _ManageOwnersState extends ConsumerState<ManageOwners> {

  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();

  String? uidOfUserBeingRemoved;
  String? uidOfUserBeingAdded;
                ...
                child: AppBarCircularActionButton(
                  onTap: () async {
                    showModalBottomSheet(
                        ...
                        context: context,
                        builder: (context) => _addOwner(group));
                  },
      ...
      body: groupAsyncValue.when(
        data: (group) {
          return SEAUserSearch(
            ...
            filter: (user) {
              return group.ownerIDs.contains(user.id);
            },
              ...
              trailing: uidOfUserBeingRemoved != null && uidOfUserBeingRemoved == user.id ? const CupertinoActivityIndicator(radius: 7.5) : ZAPButton(
                ...
                onPressed: () async {
                  if(group.creatorID != user.id) {
                    ...
                    await PermissionsManager.removeOwnerFromGroup(group, user.id);
                    setState(() {
                      uidOfUserBeingRemoved = null;
                    });
                    notifier.data.remove(user);
                    final snackBar = SnackBar(
                      content: Text("${user.firstName} is no longer an owner."),
                    );
  ...
  Widget _addOwner(GroupModel groupModel) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SEAUserSearch(
            ...
            listPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10),
            separator: const Divider(height: 20),
            filter: (user) => !groupModel.ownerIDs.contains(user.id),
            listTile: (user, notifier) => ZAPListTile(
              leading: CircleNetworkImage(
                imageURL: user.profileImageURL,
                fit: BoxFit.cover,
              ...
              horizontalTitleGap: 10,
              title: uidOfUserBeingAdded != null && uidOfUserBeingAdded == user.id ? const CupertinoActivityIndicator() : Text(
                '${user.firstName} ${user.lastName}',
                style: GoogleFonts.inter(
                    fontSize: 16,
                    fontWeight: FontWeight.w500,
              ...
              trailing: ZAPButton(
                ...
                onPressed: () async {
                  setState(() {
                    uidOfUserBeingAdded = user.id;
                  });
                  await PermissionsManager.addOwnerToGroup(groupModel, user.id);
                  setState(() {
                    uidOfUserBeingAdded = null;
                  });
                  setState(() {
                    notifier.data.add(user);
                    notifier.setLoading(true);
                  });
                  Navigator.of(context).pop();
                  // await Future.delayed(const Duration(milliseconds: 500));
                  // RoutingUtil.pushReplacementNoAnimation(context, ManageOwners(prefs: widget.prefs, groupID: groupModel.id));
                },
                child: Text(
                  'Make Owner',
                  style: GoogleFonts.inter(
