...
class WhoCanCreateEvents extends ConsumerStatefulWidget {
  final GroupModel groupModel;
  const WhoCanCreateEvents({Key? key, required this.groupModel}) : super(key: key);

  @override
  ConsumerState<WhoCanCreateEvents> createState() => _WhoCanCreateEventsState();
}

class _WhoCanCreateEventsState extends ConsumerState<WhoCanCreateEvents> {
            ...
            Padding(
              ...
              child: AppBarCircularActionButton(
                onTap: () async {
                  ...
                  RoutingUtil.pushAsync(context, fullscreenDialog: true, AddUserWhoCanCreateEvents(groupModel: widget.groupModel, groupPermissionsModel: permissions));
                },
        ...
        body: groupPermissionsAsyncValue.when(
          data: (permissions) {
            return SEAUserSearch(
                ...
                filter: (user) {
                  if(permissions.canCreateEvents.contains(user.id)
                  ...
                  return ZAPListTile(
                    ...
                    trailing: uidOfUserBeingRemoved != null && uidOfUserBeingRemoved == user.id ? const CupertinoActivityIndicator(radius: 7.5) : ZAPButton(
                      ...
                      onPressed: () async {
                        ...
                        await FBDatabase.removeUserWhoCanCreateEvents(permissions, user.id);
                        setState(() {
...
class AddUserWhoCanCreateEvents extends StatelessWidget {
  final GroupModel groupModel;
  final GroupPermissionsModel groupPermissionsModel;
  const AddUserWhoCanCreateEvents({Key? key, required this.groupModel, required this.groupPermissionsModel}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        iconTheme: const IconThemeData(color: Colors.black),
        title: Text(
          'Add Member Who Can Create Event',
          style: GoogleFonts.inter(
              color: Colors.black,
              fontWeight: FontWeight.w700,
              fontSize: 17
      ...
      body: Column(
        ...
        children: [
          SEAUserSearch(
            ...
            filter: (user) {
              if(!groupPermissionsModel.canCreateEvents.contains(user.id) && groupModel.memberIDs.contains(user.id)) {
                return true;
              }
              return false;
            },
            listTile: (user, notifier) => InkWell(
              ...
              onTap: () async {
                await FBDatabase.addUserWhoCanCreateEvents(groupPermissionsModel, user.id);
                Navigator.of(context).pop();
                Navigator.of(context).pop();
                RoutingUtil.pushAsync(context, WhoCanCreateEvents(groupModel: groupModel));
              },
              child: Container(
