...
class GroupEventsList extends ConsumerStatefulWidget {
  final Tuple4<SEAUser?, bool?, String?, String?> query;
  final bool isMainFeed;
  final SEAUser user;
  final AppConfiguration prefs;
  const GroupEventsList({Key? key, required this.query, required this.isMainFeed, required this.user, required this.prefs}) : super(key: key);

  @override
  ConsumerState<GroupEventsList> createState() => _GroupEventsListState();
}

class _GroupEventsListState extends ConsumerState<GroupEventsList> {

  @override
  void initState() {
    super.initState();
    final fp = ref.read(groupEventsProvider);
  ...
  Widget build(BuildContext context) {
    final cb = ref.watch(groupEventsProvider);

    return RefreshIndicator(
      onRefresh: () async {
        ref.read(groupEventsProvider).onRefresh(mounted, widget.query);
      },
